...
describe('StorageService', () => {
  ...
  describe('windgram cache', () => {
    const windgramId = 'test-windgram';
    const dayOffset = 2;

    it('should return null when no windgram cache exists', () => {
      expect(storageService.getCachedWindgram(windgramId, dayOffset)).toBe(null);
    });

    it('should save and retrieve windgram cache', () => {
      const cacheData = {
        url: 'https://example.com/windgram.jpg',
        expiresAt: Date.now() + (30 * 60 * 1000), // 30 minutes from now
        cachedAt: Date.now()
      };

      storageService.setCachedWindgram(windgramId, dayOffset, cacheData);
      const cached = storageService.getCachedWindgram(windgramId, dayOffset);

      expect(cached).not.toBe(null);
      expect(cached?.url).toBe(cacheData.url);
      expect(cached?.expiresAt).toBe(cacheData.expiresAt);
      expect(cached?.cachedAt).toBe(cacheData.cachedAt);
    });

    it('should validate cache is not expired', () => {
      const cacheData = {
        url: 'https://example.com/windgram.jpg',
        expiresAt: Date.now() + (30 * 60 * 1000), // 30 minutes from now
        cachedAt: Date.now()
      ...
      expect(storageService.isWindgramCacheValid(windgramId, dayOffset)).toBe(true);
    });
    ...
    it('should return false for expired cache', () => {
      ...
      expect(storageService.isWindgramCacheValid(windgramId, dayOffset)).toBe(false);
    });
    ...
    it('should clear specific windgram cache', () => {
      const cacheData = {
        url: 'https://example.com/windgram.jpg',
      ...
      storageService.clearWindgramCache(windgramId, dayOffset);

      expect(storageService.getCachedWindgram(windgramId, dayOffset)).toBe(null);
      expect(storageService.isWindgramCacheValid(windgramId, dayOffset)).toBe(false);
    });

    it('should handle different windgram IDs independently', () => {
      const cacheData1 = {
        url: 'https://example.com/windgram1.jpg',
        expiresAt: Date.now() + (30 * 60 * 1000),
        cachedAt: Date.now()
      };
      const cacheData2 = {
        url: 'https://example.com/windgram2.jpg',
        expiresAt: Date.now() + (30 * 60 * 1000),
        cachedAt: Date.now()
      };

      storageService.setCachedWindgram('windgram1', dayOffset, cacheData1);
      storageService.setCachedWindgram('windgram2', dayOffset, cacheData2);

      expect(storageService.getCachedWindgram('windgram1', dayOffset)?.url).toBe(cacheData1.url);
      expect(storageService.getCachedWindgram('windgram2', dayOffset)?.url).toBe(cacheData2.url);
    });

    it('should handle different days independently', () => {
      const cacheData0 = {
        url: 'https://example.com/day0.jpg',
        expiresAt: Date.now() + (30 * 60 * 1000),
        cachedAt: Date.now()
      };
      const cacheData1 = {
      ...
      storageService.setCachedWindgram(windgramId, 0, cacheData0);
      storageService.setCachedWindgram(windgramId, 1, cacheData1);

      expect(storageService.getCachedWindgram(windgramId, 0)?.url).toBe(cacheData0.url);
      expect(storageService.getCachedWindgram(windgramId, 1)?.url).toBe(cacheData1.url);
    });

    it('should use forecast date in cache key (not just offset)', () => {
      const cacheData = {
        url: 'https://example.com/windgram.jpg',
        expiresAt: Date.now() + (30 * 60 * 1000),
        cachedAt: Date.now()
      ...
      // Day 0 = today's date
      storageService.setCachedWindgram(windgramId, 0, cacheData);

      // Calculate expected date
      const today = new Date().toISOString().split('T')[0];
      const expectedKey = `windgram_cache_${windgramId}_${today}`;

      // Verify the key exists in localStorage
      expect(localStorageMock.getItem(expectedKey)).not.toBe(null);
    });

    it('should invalidate cache when day rolls over (midnight boundary)', () => {
      const windgramId = 'test-spot';
      const dayOffset = 0; // "today"

      // Mock Date to simulate yesterday at 11:30 PM
      const originalDate = global.Date;
      const yesterdayAt11PM = new Date('2025-01-15T23:30:00');

      // Create a mock date constructor
      const MockDateYesterday = function(this: any, ...args: any[]) {
        if (args.length === 0) {
          return new originalDate(yesterdayAt11PM.getTime());
        } else {
          return new originalDate(...(args as [number]));
        }
      } as any;
      MockDateYesterday.now = () => yesterdayAt11PM.getTime();
      MockDateYesterday.prototype = originalDate.prototype;
      Object.setPrototypeOf(MockDateYesterday, originalDate);
      global.Date = MockDateYesterday;

      // Cache windgram for "today" (Jan 15)
      const cacheData = {
        url: 'https://example.com/windgram-yesterday.jpg',
        expiresAt: Date.now() + (6 * 60 * 60 * 1000), // 6 hours from yesterday 11:30 PM
        cachedAt: Date.now()
      };
      storageService.setCachedWindgram(windgramId, dayOffset, cacheData);

      // Now mock Date to simulate today at 3:00 AM (Jan 16)
      const todayAt3AM = new Date('2025-01-16T03:00:00');

      const MockDateToday = function(this: any, ...args: any[]) {
        if (args.length === 0) {
          return new originalDate(todayAt3AM.getTime());
        } else {
          return new originalDate(...(args as [number]));
        }
      } as any;
      MockDateToday.now = () => todayAt3AM.getTime();
      MockDateToday.prototype = originalDate.prototype;
      Object.setPrototypeOf(MockDateToday, originalDate);
      global.Date = MockDateToday;

      // Request "today" (Jan 16) - should NOT find yesterday's cache (Jan 15)
      const isValid = storageService.isWindgramCacheValid(windgramId, dayOffset);

      // Expected: cache should be invalid because different calendar day
    ...
    it('should validate cache structure', () => {
      ...
      // Manually set invalid data (missing required fields)
      const forecastDate = new Date().toISOString().split('T')[0];
      localStorageMock.setItem(`windgram_cache_${windgramId}_${forecastDate}`, JSON.stringify({
        url: 'https://example.com/windgram.jpg'
        // Missing expiresAt and cachedAt
      }));

      expect(storageService.getCachedWindgram(windgramId, 0)).toBe(null);
      expect(consoleWarnSpy).toHaveBeenCalled();
    ...
    it('should handle localStorage errors gracefully on set', () => {
      ...
      expect(() => storageService.setCachedWindgram(windgramId, dayOffset, cacheData)).not.toThrow();
      expect(consoleWarnSpy).toHaveBeenCalled();
    ...
    it('should handle localStorage errors gracefully on clear', () => {
      ...
      expect(() => storageService.clearWindgramCache(windgramId, dayOffset)).not.toThrow();
      expect(consoleWarnSpy).toHaveBeenCalled();
    ...
    it('should handle malformed JSON in cache', () => {
      ...
      const forecastDate = new Date().toISOString().split('T')[0];
      localStorageMock.setItem(`windgram_cache_${windgramId}_${forecastDate}`, 'invalid json{');
